<template>
  <div>
    <div style="margin-bottom: 5px">
      <el-button v-if="data.user.role === 'employee'" type="primary" plain @click="handleAdd">提交请假申请</el-button>
    </div>
    ...
    <div class="card" style="margin-bottom: 5px;">
      <el-table :data="data.tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column label="请假人" prop="ename"></el-table-column>
        <el-table-column label="请假标题" prop="title"></el-table-column>
        <el-table-column label="请假理由" prop="text"></el-table-column>
    ...
    <el-dialog title="请假信息" v-model="data.formVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form ref="formRef":model="data.form" :rules="data.rules" label-width="80px"  style="padding: 20px 30px">
        <el-form-item label="标题" prop="title"  v-if="data.user.role === 'employee'" >
          <el-input v-model="data.form.title" placeholder="请输入标题"></el-input>
        </el-form-item>
        <el-form-item label="请假理由" prop="text"  v-if="data.user.role === 'employee'">
        ...
        <el-form-item label="审核说明" prop="reason"  v-if="data.user.role === 'admin' && data.form.status==='审核拒绝'">
          <el-input  v-model="data.form.reason" autocomplete="off" placeholder="请输入拒绝理由 "></el-input>
       </el-form-item>
      </el-form>
      <template #footer>
...
const handleAdd = () => {
  ...
  data.form.ename= data.user.name
...
// 导出
const exp = () => {
  location.href = 'http://localhost:8080/qjrecord/export?token=' + data.user.token
}
